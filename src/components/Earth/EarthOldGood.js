...
class Earth extends React.Component {
  ...
  constructor (props, context) {
    ...
    this.texture4 = <texture url={'galaxy_starfield.png'} slot={'map'} />
    this.texture1 = <texture url={'earthmap1k.jpg'} />
    this.texture2 = <texture url={'earthbump1k.jpg'} slot={'bumpMap'} />
    this.texture3 = <texture url={'earthspec1k.jpg'} slot={'specularMap'} />
    ...
    this._onAnimate = () => {
      ...
      if (this.state.primaryMarkerPosition >= this.travelPath.length ||
        typeof this.state.primaryMarkerPosition === 'undefined') {
        positionToSet = 0
      }
      this.setState({
    ...
    this.addMarker = (locationObject) => {
      ...
      var markerPosition = this.convertLatLonToVec3(
        locationObject['lat'], locationObject['lon']).multiplyScalar(this.earthRadius)
      var marker = (
        <mesh
    ...
    this.createSphereArcs = () => {
      ...
      while (i < this.destinations.length - 1) {
        let fromVector = this.convertLatLonToVec3(
          this.destinations[i]['lat'], this.destinations[i]['lon']).multiplyScalar(this.earthRadius)
        let toVector = this.convertLatLonToVec3(
          this.destinations[i + 1]['lat'], this.destinations[i + 1]['lon']).multiplyScalar(this.earthRadius)
    ...
    this.map = (x, inMin, inMax, outMin, outMax) => {
